...
namespace ch 
{
	...
	void GameMap::onRoad() 
	{
		for (size_t i = 0; i < 11; i++)
		{
			...
			{
				if (mTiles[i][j]->GetMapBaseCode() == 99 && mTiles[i][j]->GetTileType() == 0) //길 코드 이면서 그위에 tile이 없을때
				{
	...
	void GameMap::aroundRoad()//길가
	{
		for (size_t i = 0; i < 11; i++)
		{
			for (size_t j = 0; j < 21; j++)
			{
				if (mTiles[i][j]->GetMapBaseCode() == 10 && mTiles[i][j]->GetTileType() == 0) //길가 코드 이면서 그위에 tile이 없을때
				{
					TileMapObject* gameObj = ch::object::Instantiate<Place>(MapPosCalc(i, j), eColliderLayer::TilePlace); // 무덤
	...
	void GameMap::exAroundRoad()//길가를 제외한 모두
	{
		for (size_t i = 0; i < 11; i++)
		{
			for (size_t j = 0; j < 21; j++)
			{
				if (mTiles[i][j]->GetMapBaseCode() == 0 && mTiles[i][j]->GetTileType() == 0) //기본 코드 이면서 그 위에 tile이 없을때
				{
					TileMapObject* gameObj = ch::object::Instantiate<Place>(MapPosCalc(i, j), eColliderLayer::TilePlace); // 무덤
					gameObj->SetTileBase(100);
					mTiles[i][j] = gameObj;
	...
	void GameMap::aroundRoadCheck()//길주변 체크
	{
		for (size_t i = 0; i < 11; i++)
		{
			for (size_t j = 0; j < 21; j++)
	...
	void GameMap::aroundVilageCheck()//마을 체크 
	{
		for (size_t i = 0; i < 11; i++)
		{
			for (size_t j = 0; j < 21; j++)
	...
	void GameMap::initOnRoad() 
	{
		for (size_t i = 0; i < 11; i++)
		{
			for (size_t j = 0; j < 21; j++)
			{
				if (mTiles[i][j]->GetTileType() == 99) //길은 tile type이 99이다.
				{
					TileMapObject* gameObj = ch::object::Instantiate<BasePlace>(MapPosCalc(i, j), eColliderLayer::BackGround); // 무덤
					gameObj->SetMapBaseCode(99);
					mTiles[i][j] = gameObj;
